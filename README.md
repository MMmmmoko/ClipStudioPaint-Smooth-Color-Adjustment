...
效果展示视频:
https://www.bilibili.com/video/BV1Y9U2BkEZz
...
随着CSP版本的更新，较新版本的插件会放弃测试过旧的版本（一台电脑实在没办法同时装太多的CSP和优动漫版本）

### CSP

此工具设计为**可以用于未来版本的CSP**，但是在使用未来版本的CSP时需要对功能是否可用进行一次验证。因为工具在生成关于新版本的信息时，即使没有报错也不代表CSP没有修改底层逻辑，这些无法预料的底层修改是可能造成本工具提供的功能产生错误的。测试的版本：4.0.3、4.1.4、4.2.0、4.2.2、5.0.0
...
现在已经支持绝大部分功能，优动漫的缩时摄影无法正确导出较大图像尺寸的视频（能超过1280px一些，但是超过太多了不行，属于3.0早期版本的bug，在优动漫4.0.5版本已修复。），测试的版本：3.0.6PRO体验版、EX版；4.0.5PRO。
...
插件的跨未来版本能力在面对第一次CSP升级考验下失败了..这一次4.2.0更新CSP内部代码变动比较大，导致之前版本插件的色彩平衡、曲线、插件解锁等功能有部分失效。但我还是对此插件的跨未来版本的能力保持有信心，因为CSP在后续的小型更新中应该不会再有这么大面积改动底层代码的情况。这次4.2.0更新可以看到CSP已经开始着手处理部分场景下的流畅性了，很期待CSP后续的流畅性改善。

5.0.0版本大部分基础代码应该是4.2.0就写好了，没有出现升级后插件需要更新才能使用5.0.0的情况，这次插件的跟随更新只是修复了一下曲线功能失效的问题。

5.0.0塞了很多新功能，但CELSYS依然没有去动调色预览，我不明白。
